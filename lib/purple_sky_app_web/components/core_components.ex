defmodule PurpleSkyAppWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: PurpleSkyAppWeb.Gettext
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between gap-6", @class]}>
      <div>
        <h1 class="font-extrabold text-purple-500 dark:text-purple-400">
          {render_slot(@inner_block)}
        </h1>
        <p :if={@subtitle != []} class="mt-2 text-lg font-semibold dark:text-slate-300 text-slate-500">
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(PurpleSkyAppWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(PurpleSkyAppWeb.Gettext, "errors", msg, opts)
    end
  end
